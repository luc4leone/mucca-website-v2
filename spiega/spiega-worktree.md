...
## Esempio pratico: confrontare due temi

Stesso meccanismo, applicato a un cambio di tema (i token in `css/tokens.css`) invece che a un componente o una sezione.

È un caso particolarmente pulito: per definizione, cambiare identità visiva tocca **solo** `tokens.css`, non markup né componenti (vedi `design-system.md`).
